...
import type { SubscriptionFilter, SubscriptionCreateDescriptor, SubscriptionCreateMessage } from '../types/subscription-types.js';
...
export class Subscription {
  ...
  public static async decrypt(
    recordsWrite: SubscriptionCreateMessage,
    ...
    const fullDerivationPath = Subscription.constructKeyDerivationPath(matchingEncryptedKey.derivationScheme, recordsWrite);

    // NOTE: right now only `ECIES-ES256K` algorithm is supported for asymmetric encryption,
    // so we will assume that's the algorithm without additional switch/if statements
    const leafPrivateKey = await Subscription.derivePrivateKey(ancestorPrivateKey, fullDerivationPath);
  ...
  public static constructKeyDerivationPath(
    ...
    recordsWriteMessage: SubscriptionCreateMessage
  ): string[] {
    ...
    if (keyDerivationScheme === KeyDerivationScheme.DataFormats) {
      fullDerivationPath = Subscription.constructKeyDerivationPathUsingDataFormatsScheme(descriptor.schema, descriptor.dataFormat);
    } else if (keyDerivationScheme === KeyDerivationScheme.ProtocolPath) {
      fullDerivationPath = Subscription.constructKeyDerivationPathUsingProtocolPathScheme(descriptor);
    } else if (keyDerivationScheme === KeyDerivationScheme.ProtocolContext) {
      fullDerivationPath = Subscription.constructKeyDerivationPathUsingProtocolContextScheme(contextId);
    } else {
      // `schemas` scheme
      fullDerivationPath = Subscription.constructKeyDerivationPathUsingSchemasScheme(descriptor.schema);
    }
   ...
   */
  public static constructKeyDerivationPathUsingProtocolPathScheme(descriptor: SubscriptionCreateDescriptor): string[] {
  ...
  public static async derivePrivateKey(ancestorPrivateKey: DerivedPrivateJwk, fullDescendantDerivationPath: string[]): Promise<Uint8Array> {
    ...
    Subscription.validateAncestorKeyAndDescentKeyDerivationPathsMatch(ancestorPrivateKeyDerivationPath, fullDescendantDerivationPath);
  ...
  /**
   *  Converts an incoming SubscriptionFilter into a Filter usable by MessageStore.
   *
   * @param filter A SubscriptionFilter
   * @returns {Filter} a generic Filter able to be used with MessageStore.
   */
  public static convertFilter(filter: SubscriptionFilter): Filter {
