...
import { SubscriptionCreate } from '../interfaces/subscription-create.js';
...
export class SubscriptionCreateHandler implements MethodHandler {
  ...
  public async handle({
    ...
    let subscriptionCreate: SubscriptionCreate
    try {
      subscriptionCreate = await SubscriptionCreate.parse(message);
    } catch (e) {
    ...
    try {
      ...
      await subscriptionCreate.authorize(tenant, this.messageStore);
    } catch (e) {
    ...
    const newMessageIsInitialWrite = await subscriptionCreate.isInitialWrite();
    if (!newMessageIsInitialWrite) {
      try {
        const initialWrite = await SubscriptionCreate.getInitialWrite(existingMessages);
        SubscriptionCreate.verifyEqualityOfImmutableProperties(initialWrite, message);
      } catch (e) {
        return messageReplyFromError(e, 400);
      }
    }
    ...
    const indexes = await constructRecordsWriteIndexes(subscriptionCreate, isLatestBaseState);
  ...
  public async processEncodedData(
    ...
    SubscriptionCreateHandler.validateDataIntegrity(message.descriptor.dataCid, message.descriptor.dataSize, dataCid, dataBytes.length);
  ...
  public async putData(
    ...
    try {
      SubscriptionCreateHandler.validateDataIntegrity(message.descriptor.dataCid, message.descriptor.dataSize, result.dataCid, result.dataSize);
    } catch (error) {
...
export async function constructRecordsWriteIndexes(
  recordsWrite: SubscriptionCreate,
  ...
  const indexes: Record<string, any> = {
    ...
    recordId: message.recordId,
    entryId: await SubscriptionCreate.getEntryId(recordsWrite.author, recordsWrite.message.descriptor)
  };
